...
`setWorkState(start=true, mode=1)` (cmd `0x02`, payload `01 01` — первым
идёт режим, вторым флаг пуска):
...
### 5.1 Умолчания повторов

`BaseDevice.sendHandle` объявлен как
`{bool resend = false, int resendTimes = 2, int timeout = 5, bool withCompleter = true}`
(0x55ee28), но `PcmDevice.sendHandle` (0x55ec04) переопределяет их на
**`resend = true`, `timeout = 4`**. То есть для кофемашины повтор включён
всегда: до трёх отправок, каждая ждёт ответа 4 секунды.

Семафор — `LocalSemaphore(1)` с очередью `ListQueue<Completer>`, создаётся в
конструкторе `BaseDevice` (0x54b434). Одновременная транзакция ровно одна.

### 5.2 Что оригинал НЕ делает

Фазу телеметрии он не отслеживает и под неё не подстраивается:

* время в приёмном пути он берёт ровно один раз и никуда не кладёт:
  конструктор `PcmBlePack` (0x867294) подставляет `DateTime` в строку
  `debugPrint("接收 Receiving data: … ,receiving time=…")`. Между кадрами
  ничего не переносится, `_getCurrentMicros` там нет — момент прихода
  прошлого кадра приёмнику неизвестен;
* `writeData` помнит только время **своей** прошлой записи (`field_2f`), и
  только ради 20-миллисекундного пола;
* среди 46 констант `Duration` во всём приложении нет ни одной около 1020 или
  1050 мс — то есть периода телеметрии в коде нет ни в каком виде;
* `Timer.periodic` в устройственном слое нет: телеметрию машина шлёт сама,
  оригинал её не опрашивает.

Вся его защита от потерь — сериализация, пол между записями, таймаут и повтор.

Не проверяет он и то, ответила ли машина вообще. `try`/`catch` внутри
`PcmDevice.initConnect` нет ни одного, а `getTempSetting`/`getDeviceWorkParam`
он повторяет ровно в одном месте — после успешной записи
(`_onSetDeviceWorkParam`, 0x865194). Молчащая машина его не смущает, потому что
все диапазоны у него заполнены ещё до BLE: конструктор `PcmDevice` (0x54b110 …
0x54b1e4) кладёт `SoakingTime(0, 20, 0)`, `ExtractionTime(60, 360, 60)`,
`ExtractionPressure(0, 4, 0)` и `DeviceMaxMinValue(208.0, 100.0)`. С PCM03SMAX
эти числа не совпадают ни в одной строке — её собственный ответ
`0x17 07 05 00 0a 05 00 0a 1e 1e fa 01 0f` даёт 0–10 / 0–10 / 30–250 / 1–15, —
так что до ответа машины правка в оригинале идёт по чужим границам.

### 5.3 Замеры на живой машине (PCM03SMAX)

По трассе нашего клиента, 427 пар «запрос → ответ»:

| | |
|---|---|
| медиана ответа | 89 мс |
| p95 / p99 | 209 / 239 мс |
| максимум (кроме `0x02`) | 260 мс |
| период телеметрии | 1.021 с (82 037 интервалов), внутри сессии sd 10–20 мс |
| первый кадр `0x00` | через ≈1.7 с после подключения |

Потери: 26 на 455 отправок. Из них **22 — раньше первого кадра телеметрии**,
то есть в рукопожатии (8.8 % отправок против 2.0 % дальше). Чаще всего гибнет
самый первый кадр сессии — в 6 сессиях из 38.
...
| `0x02` | `setWorkState` | → | 2 байта: `[WorkMode, start]` | эхо |
...
| 5 | состояние таймера: `0` — выключен, `1` — взведён, **`2` — отработал** |

Байт 5 не булев. Снято с живой машины (PCM03SMAX): взвели `02 10 1c 01 01 01`
(«только пролив», 16:28), будильник сработал, машина сама выполнила задачу — и
на следующем `0x24` вернула `02 10 1c 01 00 02`. Это единственный способ узнать,
что задача отработала: отдельного кадра машина не шлёт, а `0x00` про таймер
молчит. Узнать можно только спросив `0x24`.

Оригинальное приложение сравнивает байт ровно с единицей
(`pcm_device.dart`, `0x866bc4: cmp x0, #1`), поэтому `2` показывает как
«выключен» и разницы между «не заводили» и «отработал» не видит. Обратно в
машину пишутся только `0` и `1`.

Обратный отсчёт до срабатывания оригинал ведёт сам, на часах телефона
(`redux/appoint.dart` — `Timer`, `DateTime`, ни одной отправки в BLE), и по нулю
локально ставит `isEnable = false`, машине об этом не сообщая.
...
Байты идут в порядке `[WorkMode, start]`. В снапшоте порядок аргументов
`setWorkState(start, mode)` сохранился, а порядок записи в буфер — нет; живая
машина показала обратное: `02 01 00` (нагрев со старым порядком) она понимала
как стоп режима 1 и не делала ничего, а `02 01 01` и `02 01 02` одинаково
запускали нагрев с проливом — второй байт читался как флаг пуска.
...
7. Запуск: setWorkState -> 7F 00 07 CS 02 <mode> 01,
