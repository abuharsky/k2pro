# Публикация BetterCup — автоматизация и ключи
...
только **BetterCup**. Это и есть ограничение радиуса: ключ этого пользователя не
...
официальное приложение — «Happygo Cera»). Поэтому оно называется нейтрально —
**BetterCup**, а «K2 Pro» остаётся только как модель самой машины в тексте.
Это снимает основной риск отклонения на ревью (Guideline 4.1/5.2 — чужой
товарный знак): имя приложения не повторяет чужой бренд. В описании честно
указываем «unofficial companion». Пайплайн от имени не зависит — оно живёт в
`CFBundleDisplayName` и метаданных, не в подписи (bundle id остаётся
`ru.bukharskiy.k2pro`).
