...
class SceneState {
  ...
  int get _tank {
    ...
    return switch (phase) {
      ...
      // Спираль светится, только пока её питают. Остаточное тепло нагревом не
      // считается: машина остывает десятки минут, и раньше сцена всё это время
      // стояла «в нагреве» — в том числе после остановки цикла.
      BrewPhase.heating ||
      BrewPhase.preInfusion ||
      BrewPhase.standstill => live && _heats ? 1 : 0,
      _ => 0,
    };
  }
  ...
  int get _puck => switch (phase) {
    // Смачивание видно с середины фазы: вода доходит до таблетки не сразу.
    BrewPhase.preInfusion => phaseFraction < 0.5 ? 0 : 1,
