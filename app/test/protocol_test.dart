...
void _tempLimitsSanity() {
  ...
  test('лесенка ожидания: быстрый первый повтор, растущее окно, потолок', () {
    ...
    // Одиночная попытка ждёт ровно столько же, сколько первая из трёх.
    // Раньше ей доставался сразу потолок — «повторять всё равно нечем, так
    // подождём подольше», — и форс-запись уставок перед пуском стоила по
    // четыре секунды на кадр: в живой трассе 25.08 кадр пуска ушёл на
    // двенадцатой секунде после нажатия.
    expect(retryTimeout(1, 1), const Duration(milliseconds: 600));
  });
}
